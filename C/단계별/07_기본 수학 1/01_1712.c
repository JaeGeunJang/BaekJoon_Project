...
출력
첫 번째 줄에 손익분기점 즉 최초로 이익이 발생하는 판매량을 출력한다. 손익분기점이 존재하지 않으면
-1을 출력한다.
*/
...
int main()
{
  ...
  scanf("%d %d %d",&base, &make, &price);

  int profit = price - make;

  if (profit <= 0) printf("%d", -1);
  else printf("%d", base/profit+1);
