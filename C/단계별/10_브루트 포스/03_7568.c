...
1 초	128 MB	48966	27152	23285	57.018%
문제
...
int main()
{
  ...
  int *weight = malloc(sizeof(int) * num);
  int *height = malloc(sizeof(int) * num);
  int *rank = malloc(sizeof(int) * num);

  for (int i = 0; i < num; i++) rank[i] = 1;

  for (int i = 0; i < num; i++) scanf("%d %d", weight+i, height+i);

  for (int i = 0; i < num; i++) {
    for (int j = 0 ; j < num; j++) {
      if (i == j) continue;
      if (weight[i] > weight[j] && height[i] > height[j]) rank[j] += 1;
    }
  }
  for (int i = 0; i < num; i++) printf("%d ", rank[i]);
}
